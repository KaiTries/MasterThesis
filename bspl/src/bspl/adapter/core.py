...
from ..protocol import Parameter, Protocol
import bspl
import bspl.adapter.jason
import bspl.adapter.schema
from bspl.utils import identity
from bspl.adapter.agent_store import Agent, AgentStore
from bspl.adapter.system_store import System, SystemStore
...
class Adapter:
    def __init__(
        ...
        **kwargs,
        ...
        self.protocols: dict[str, Protocol] = {s["protocol"].name : s['protocol'] for s in systems.values()}
        self.systems: SystemStore = SystemStore(systems)

        self.agents: AgentStore = AgentStore(agents)
        self.addresses = self.agents.get_agent(self.name).addresses
    ...
    def add_system(self, name: str, system: dict):
        ...
        # Update roles
        self.roles.update(
            {r for r in system["roles"] if system["roles"][r] == self.name}
        )
        # Inject schema methods into new protocol messages
        if system["protocol"].name not in self.protocols:
            self.protocols[system['protocol'].name] = system["protocol"]
            self.inject(system["protocol"])
            # Update messages
            for message in system["protocol"].messages.values():
                self.messages[message.qualified_name] = message
        # Update history and enabled messages stores
        self.history.add_context(name)
        self.enabled_messages.add_context(name)
    ...
    def reassign_role(self, system_name: str, role: str, agent_name: str):
        ...
        system = self.systems.get_system(system_name)
        if not system:
            raise ValueError(f"System {system_name} does not exist.")
        if role not in system.roles:
            raise ValueError(f"Role {role} does not exist in system {system_name}.")
        old_agent = system.roles[role]
        system.roles[role] = agent_name
        # Update roles set
        if old_agent == self.name and agent_name != self.name:
            self.roles.remove(role)
        elif old_agent != self.name and agent_name == self.name:
            self.roles.add(role)
    ...
    async def send(self, *messages):
        def prep(message):
            ...
            if hasattr(message, "_dest") and message._dest:
                ...
                system = self.systems.get_system(message.system)
                # Create a copy for each recipient
                for recipient_role in message.schema.recipients:
                    recipient_agent = system.roles[recipient_role.name]
                    agent_endpoints = self.agents.get_agent(recipient_agent).addresses
    ...
    def register_generators(self, handler, schemas, options={}, index=None):
        if schemas in self.generators:
            gs = self.generators[schemas[0]]
            if handler not in gs:
                gs.insert(index if index is not None else len(gs), handler)
        else:
            self.generators[schemas[0]] = [handler]

    def get_schema_for_tup(self, msg_name: str):
    ...
    async def handle_enabled(self, message):
        ...
        for tup in self.generators.keys():
            schemas = self.get_schema_for_tup(tup)
            for group in zip(*(schema.match(message) for schema in schemas)):
    ...
    def start(self, *tasks, use_uvloop=True):
        if use_uvloop:
            try:
                ...
                use_uvloop = False
        self.running = True
        aiorun.run(self._boot(*tasks), stop_on_unhandled_errors=True, use_uvloop=use_uvloop)
    ...
    def construct_initiators(self):
        # Add initioators
        for sID, s in self.systems.systems.items():
    ...
    def compute_enabled(self, observations):
        ...
        for o in observations:
            for schema in self.systems.get_system(o.system).protocol.messages.values():
